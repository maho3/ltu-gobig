...
struct Lightcone
{
    ...
    // }
        void _finalize(bool apply_fibcoll = true)
    {
        if (snap_indices_done.size() != Nsnaps)
            throw std::runtime_error("not all snapshots have been added");
        ...
        if (do_downsample)
        {
            double fibcoll_rate = 0.0;
            
            // get an idea of the fiber collision rate in our sample,
            // so the subsequent downsampling is to the correct level.
            // This is justified as all this stuff is not super expensive.
            fibcoll_rate = fibcoll</*only_measure=*/true>();

            // first downsampling before fiber collisions are applied
            if (verbose) std::printf("downsample\n");
            downsample(fibcoll_rate + (apply_fibcoll ? Numbers::fibcoll_rate_correction : 0.0));
        }
    ...
    // Modified public finalize method
    pyb::tuple finalize(bool apply_fibcoll = true)
    {
        _finalize(apply_fibcoll);
    ...
    // Get current galaxy data as numpy arrays (without processing)
    pyb::tuple get_current_data() const {
        pyb::array_t<double> RAnumpy = pyb::array(RA.size(), RA.data()),
                            DECnumpy = pyb::array(DEC.size(), DEC.data()),
                            Znumpy = pyb::array(Z.size(), Z.data());
        pyb::array_t<galid_t> GALIDnumpy = pyb::array(GALID.size(), GALID.data());
    ...
    double measure_fibcoll() { 
        return fibcoll<true>(); 
    }

    double apply_fibcoll() { 
        return fibcoll<false>(); 
    }

    double fibcoll_wrapper(bool only_measure = true) {
        return only_measure ? fibcoll<true>() : fibcoll<false>();
    }

    // Wrapper method for downsample (already public, just exposing it)
    void downsample_wrapper(double plus_factor = 0.0) {
        downsample(plus_factor);
    }
...
PYBIND11_MODULE(lc, m)
{
    ...
    pyb::class_<Lightcone> (m, "Lightcone")
        ...
        .def("measure_fibcoll", &Lightcone::measure_fibcoll, 
             "Measure fiber collision rate without applying collisions")
        .def("apply_fibcoll", &Lightcone::apply_fibcoll,
             "Apply fiber collisions and return collision rate") 
        .def("fibcoll", &Lightcone::fibcoll_wrapper,
             "Apply or measure fiber collisions based on parameter",
             pyb::arg("only_measure") = true)
        .def("downsample", &Lightcone::downsample_wrapper,
             "Downsample galaxies to target distribution",
             pyb::arg("plus_factor") = 0.0)
        .def("get_current_data", &Lightcone::get_current_data,
             "Get current galaxy data as (RA, DEC, Z, GALID) tuple");
}
